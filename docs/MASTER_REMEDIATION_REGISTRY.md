...
|---|---|---|---|---|
| **EB-1** | `QA_SERVICE` (a scoped QA service-role key) exists in no working copy | `npm run test:security` (188 assertions), `test:ai` live half, `setup-identities.mjs`. **Phase 1's evidence cannot be re-verified, and §4.2 proves it has drifted** | whoever holds the QA keys | 1 |
