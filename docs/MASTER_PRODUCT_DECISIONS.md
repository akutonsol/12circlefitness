...
| **PD-A05** | **Who may read a decision trace?** Migration 089 grants every `coach` read of **every** trace platform-wide; every sibling table chose "active coach or admin". 12 Circle allows self-serve coach signup, so `coach` is not a trusted class | `F-J-12`, `E-04`, `E-16`, `ENG-22` | (a) subject + their active coach + admin; (b) (a) plus `content_manager` for engine QA; (c) status quo | **(a).** Adopt (b) only with a written, time-bounded reason, and never into production | **No** — one policy predicate, but it changes what platform staff can see | Julia + privacy | 2 |
